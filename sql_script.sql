...
create table if not exists public.caretaker(caretaker_id SERIAL primary key, name varchar not null, phone varchar not null, 
street varchar not null, caretaker_zip_ref varchar references city);

create table if not exists public.vet(vet_id SERIAL primary key, cvr varchar not null, name varchar not null, 
phone varchar not null, street varchar not null,
    vet_zip_ref varchar references city);

create table if not exists public.Pet(pet_id serial primary key, name varchar not null, age int not null, 
...
create table if not exists public.Bird(bird_id integer references pet, singing_frequency integer not null);
